...
function renderApp(state) {
  ...
  appNodes.controls.classList.toggle('hide', true)
...
function getRandomShellSize(s = 0) {
  const baseSize = s == 0 ? shellSizeSelector() : s;
...
function seqStartShell() {
  const size1 = getRandomShellSize(5);
  const size2 = getRandomShellSize(5);
  const shell1 = new Shell(ringShell(size1.size));
  const shell2 = new Shell(ringShell(size2.size));
...
function seqSmallBarrage(shellType) {
  seqSmallBarrage.lastCalled = Date.now();
  const barrageCount = IS_DESKTOP ? 11 : 5;
  const specialIndex = IS_DESKTOP ? 3 : 1;
  const shellSize = Math.max(0, shellSizeSelector() - 2);
  const randomMainShell = shellType
...
function seqRandomShell() {
  ...
  const shell = new Shell(ringShell(size.size));
...
function seqSmallBarrage1() {
  seqSmallBarrage1.lastCalled = Date.now();
  const barrageCount = IS_DESKTOP ? 11 : 5;
  const specialIndex = IS_DESKTOP ? 3 : 1;
  const shellSize = Math.max(0, shellSizeSelector() - 2);
  const randomMainShell = Math.random() < 0.78 ? crysanthemumShell : ringShell;
...
seqSmallBarrage1.cooldown = 15000;
seqSmallBarrage1.lastCalled = Date.now();
...
function showDynamicText(content, fontSize = '60px', position = { x: '50%', y: '50%' }, duration = 2000) {
  // 获取文本容器
...
function startSequence() {
  ...
  const shells = [
    crackleShell, crossetteShell, fallingLeavesShell, crysanthemumShell,
    ghostShell, floralShell, horsetailShell, palmShell, ringShell,
    strobeShell, willowShell
  ]
  function seqSmall() {
    const sh = shells[Math.floor(Math.random() * shells.length)]
    return seqSmallBarrage(sh)
  }

  function multiSeqRandom(left, right) {
    let times = Math.floor(Math.random() * (right - left)) + left
    while (times-- > 0) {
      seqRandomFastShell()
    }
  }
